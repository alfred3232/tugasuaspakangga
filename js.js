...
// Initial job data (from insert_jobs.php)
const initialJobs = [
  {
    id: 1,
...
function initializeJobs() {
  ...
  if (!storedJobs) {
    // Initialize with initialJobs if localStorage is empty
...
function addJob(job) {
  // Validate job data
  if (!job.title || !job.company || !job.location || !job.type || !job.categories || !job.description || !job.url) {
    throw new Error('All fields are required');
  }
  if (!/^(https?:\/\/[^\s$.?#].[^\s]*)$/i.test(job.url)) {
  ...
  // Format salaryText
  const salaryMin = job.salary_min * 1000000;
  const salaryMax = job.salary_max * 1000000;
  const salaryText = `Rp ${salaryMin.toLocaleString('id-ID')} - Rp ${salaryMax.toLocaleString('id-ID')}`;

  // Create new job object
  const newJob = {
    id: jobs.length ? Math.max(...jobs.map(j => j.id)) + 1 : 1, // Generate new ID
    title: job.title,
    company: job.company,
    ...
    type: job.type,
    categories: job.categories.split(',').map(c => c.trim()),
